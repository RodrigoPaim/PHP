...
$stmt = $db->prepare("select * from address where client_id = :client_id");
// $clientID=array(1);
// $client=1;
...
// foreach ($clientID as $client) {
    ...
    $stmt->bindValue(':client_id', 1, PDO::PARAM_INT); //3° param is optional, once the database typify all data
    $stmt->execute();
